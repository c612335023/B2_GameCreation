...
/* static */
static CLIENT	gClients[CLIENTNUM];  /* クライアント */
...
/*****************************************************************
関数名	: RecvFloatData
機能	  : クライアントからfloat型のデータを受け取る
引数	  : int   pos	        : クライアント番号
		      float *floatData  : 受信したデータ
出力	  : 受け取ったバイト数
*****************************************************************/
int RecvFloatData(int pos, float *floatData)
{
  int n, tmp;
  ...
  n = RecvData(pos, &tmp, sizeof(float));
  (*floatData) = ntohl(tmp);
